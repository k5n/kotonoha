...
- Layer responsibilities:
  - **Presentation** (routes, components): Routes are page-level components that compose container components and delegate business logic. Container components invoke usecases and compose presentational components. Presentational components are pure UI parts without business logic. All components may access stores directly to avoid prop drilling.
