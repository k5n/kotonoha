...
- **テストフレームワーク**: Vitest (Node.js)
- **テスト構成ファイル**: `vitest.config.nodejs.ts`
...
1. **単体テスト**:
   ...
   - 例: `src/lib/domain/services/extractScript.test.ts`
   - 目的: 個別の関数やモジュールの動作を検証

**モック戦略:**

- `src/lib/domain/services` 以下の純粋関数は外部依存がないため、モックは不要。
- それ以外のモジュールでは、Vitest のモック機能を使用して外部依存（Tauri プラグインや他のモジュール）をモック化。
