...
- 各テーブルでクエリに必要な項目以外は content カラムにJSONで保存することで、DBテーブルのスキーマ変更を最小限にする
    ...
    - content カラムへの新規フィールド追加はアプリ側で柔軟に対応できる
...
1. ジャーナルモード (必須要件) 本設計ではバックグラウンドでの incremental_vacuum 実行中も UI 操作（読み取り）をブロックしないことが必須であるため、SQLite3 は WAL モード (Write-Ahead Logging) で動作させる （Tauri SQL プラグインのデフォルトで有効になっている）
2. 自動バキューム設定 (要設定) ファイル肥大化を防ぐため、テーブル作成前に以下の設定を適用
...
- 共有ストレージから他のデバイスがアップロードしたファイルのうち、
  自身が最後に同期したタイムスタンプ以降（バッファも持たせて少し前から）の自分のデバイスID以外のファイルを取得
...
同期処理中はプログレス表示して UI 操作をブロックする前提です。
ただしキャンセルは可能にし、キャンセルされた場合はトランザクションを ROLLBACK して終了します。
...
この仕様では sync_pending_changes テーブルは多くの履歴が溜まった後に削除されることがあります。
SQLite3 ではデータを削除してもファイルサイズは縮小されず、ファイルサイズを縮小するには VACUUM 処理が必要です。

VACUUM は時間のかかる処理であり、アプリの応答性に影響を与える可能性があるため、DB のマイグレーション設定に以下を設定しておきます（テーブル作成前に実行する必要があります）。
これにより、データを削除すると自動的に freelist（再利用可能リスト）には登録されますが、ファイルサイズの縮小は自動では行われません。

```sql
PRAGMA auto_vacuum = INCREMENTAL;
```

同期処理の完了後に、以下をバックグラウンドで実行します。
...
最初の `PRAGMA freelist_count;` で freelist に登録されているページ数が取得できます。
ある閾値（例えば 1000 ページなど）を超えている場合に、`PRAGMA incremental_vacuum(1000);` を実行してファイルサイズを縮小します。
引数を指定することで、一度に全てのページを縮小せずに、指定したページ数ずつ縮小できます。
