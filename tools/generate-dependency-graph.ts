...
const getFileDisplayName = (filePath: string) => {
  ...
  if (fileName.endsWith('.svelte.ts')) return fileName.replace(/\.svelte\.ts$/, '');
  if (fileName.endsWith('.svelte')) return fileName.replace(/\.svelte$/, '');
  if (fileName.endsWith('.ts')) return fileName.replace(/\.ts$/, '');
  return fileName;
};

const getFileClassName = (filePath: string) => {
  if (filePath.endsWith('.svelte.ts')) return 'cssSvelteScript';
  if (filePath.endsWith('.svelte')) return 'cssSvelteComponent';
  if (filePath.endsWith('.ts')) return 'cssTypeScript';
  return undefined;
};
...
function generateMermaidBlock(
  ...
  mermaidGraph += '    classDef cssSvelteComponent fill:#325;\n';
  mermaidGraph += '    classDef cssSvelteScript fill:#535;\n';
  mermaidGraph += '    classDef cssTypeScript fill:#352;\n';
      ...
      if (key === 'files') {
        if (level.files) {
          level.files.forEach((file: string) => {
            ...
            const className = getFileClassName(file);
            if (className && !classAssignments.has(mermaidId)) {
              classAssignments.set(mermaidId, className);
            }
          });
        }
      } else {
  ...
  for (const [mermaidId, className] of classAssignments.entries()) {
    mermaidGraph += `    class ${mermaidId} ${className}\n`;
  }
